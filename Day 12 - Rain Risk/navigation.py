...
def move(instruction: str, distance_from_start: list, facing_direction: str) -> tuple([list, str]):
	instruction_direction: str = instruction[:1]
	instruction_distance: int = int(instruction[1:])
	...
	if instruction_direction == "R" or instruction_direction == "L":
		facing_direction = rotate(facing_direction, instruction_distance, instruction_direction)
...
def rotate(starting_direction: str, rotation_degrees: int, rotation_direction: str) -> str:
...
def run_test():
	...
	final_message = f"Distance from start is {abs(distance_from_start[0])} miles {north_south} " \
	                f"and {abs(distance_from_start[1])} miles {east_west}, " \
...
def run_real():
	...
	clean_real = input_real.split("\n\t")

	distance_from_start = [0, 0]
	facing_direction = "E"

	for instruction in clean_real:
		# print(instruction, distance_from_start)
		distance_from_start, facing_direction = move(instruction, distance_from_start, facing_direction)
	# print(distance_from_start, facing_direction)
