def main():
	input_passports: str = '''pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
...
hgt:173cm byr:1925 pid:070222017 iyr:2013 hcl:#ceb3a1 ecl:gry eyr:2024'''
	...
	passport_fields_limits: dict = {
		"byr": [4, 1920, 2002],
		"iyr": [4, 2010, 2020],
		"eyr": [4, 2020, 2030],
		"hgt": {"cm": [150, 193],
		        "in": [59, 76]},
		"hcl": ["#", 6],  # regex the a-f, 0-9
		"ecl": ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"],
		"pid": [9],
	}

	total_valid_passports: int = 0

	for passport in input_passports.split("\n\n"):
		passport_data: list = passport.replace(" ", "\n").split("\n")
		if is_passport_valid(extract_passport_data(passport_data), passport_fields_limits):
			total_valid_passports += 1

	print(total_valid_passports)


def extract_passport_data(passport: list) -> dict:
	temporary_passport: dict = {}
	for data_point in passport:
		data_point_extracted = data_point.split(":")
		temporary_passport[data_point_extracted[0]] = data_point_extracted[1]
	return temporary_passport


def is_passport_valid(passport: dict, passport_field_checks: dict) -> bool:
	# cid ignored for now
	...
	for field in required_fields:
		if field in passport.keys():
			if field == "byr" or field == "iyr" or field == "eyr":
				if len(passport[field]) == passport_field_checks[field][0]:
					if passport_field_checks[field][1] <= int(passport[field]) <= passport_field_checks[field][2]:
						valid_fields += 1

			elif field == "hgt":
				height_units: str = passport[field][-2:]
				if height_units == "cm" or height_units == "in":
					height_value: int = passport[field][:len(passport[field]) - 2]
					if passport_field_checks[field][height_units][0] <= int(height_value) <= \
							passport_field_checks[field][height_units][1]:
						valid_fields += 1

			elif field == "hcl":
				if passport[field][0] == passport_field_checks[field][0]:
					only_color: str = passport[field][1:]
					if len(only_color) == passport_field_checks[field][1]:
						try:
							int(only_color, 16)
							valid_fields += 1
						except(ValueError):
				...
				if passport[field] in passport_field_checks[field]:
					valid_fields += 1

			elif field == "pid":
				if len(passport[field]) == passport_field_checks[field][0]:
					if passport[field].isnumeric():
						valid_fields += 1
